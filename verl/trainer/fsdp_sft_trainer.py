...
class FSDPSFTTrainer(object):
    ...
    def save_checkpoint(self, step):
        """Save model checkpoint with support for both full state dict and sharded checkpoints.
        
        Args:
            step: Current training step
        """
        from torch.distributed.fsdp import FullStateDictConfig, StateDictType, ShardedStateDictConfig
        import tempfile
        import os
        ...
        # Determine checkpoint type based on config
        use_sharded = self.config.model.get('use_sharded_checkpoints', False)

        if use_sharded:
            # Save sharded checkpoint using DCP
            save_dir = os.path.join(path, 'sharded_checkpoint')
            os.makedirs(save_dir, exist_ok=True)

            # Configure sharded state dict
            sharded_cfg = ShardedStateDictConfig(
                offload_to_cpu=True,
                checkpoint_prefix=os.path.join(save_dir, 'shard')
            )

            with FSDP.state_dict_type(self.fsdp_model, StateDictType.SHARDED_STATE_DICT, sharded_cfg):
                sharded_state_dict = self.fsdp_model.state_dict()
                
                # Save optimizer state if needed
                if self.config.model.get('save_optimizer', False):
                ...
                torch.save(sharded_state_dict, 
                         os.path.join(save_dir, f'model_shard_{self.device_mesh.get_rank()}.pt'))

            # Save model config and tokenizer only from rank 0
            if self.device_mesh.get_rank() == 0:
                self.model.config.save_pretrained(path)
                self.tokenizer.save_pretrained(path)
                
                # Save metadata about sharding
                metadata = {
            ...
            # Traditional full state dict saving (only from rank 0)
    ...
    def load_checkpoint(self, checkpoint_path):
        """Load model checkpoint with support for both full and sharded checkpoints.
        ...
        # Load metadata to determine checkpoint type
        metadata = torch.load(os.path.join(checkpoint_path, 'metadata.pt'))
        
        if metadata['is_sharded']:
            # Load sharded checkpoint
            sharded_dir = os.path.join(checkpoint_path, 'sharded_checkpoint')
            
            # Configure sharded state dict
            sharded_cfg = ShardedStateDictConfig(
                offload_to_cpu=True,
                checkpoint_prefix=os.path.join(sharded_dir, 'shard')
            )
            
            with FSDP.state_dict_type(self.fsdp_model, StateDictType.SHARDED_STATE_DICT, sharded_cfg):
                # Each rank loads its own shard
                shard_path = os.path.join(sharded_dir, f'model_shard_{self.device_mesh.get_rank()}.pt')
                sharded_state_dict = torch.load(shard_path)
                self.fsdp_model.load_state_dict(sharded_state_dict)
                
                # Load optimizer state if it exists
                if os.path.exists(os.path.join(sharded_dir, 'optimizer.pt')):
                    optim_state = torch.load(os.path.join(sharded_dir, 'optimizer.pt'))
                    FSDP.optim_state_dict_to_load(self.fsdp_model, self.optimizer, optim_state)
        
        else:
            # Load traditional full state dict (only on rank 0, broadcast to others)
            cfg = FullStateDictConfig(offload_to_cpu=True, rank0_only=True)
            ...
            with FSDP.state_dict_type(self.fsdp_model, StateDictType.FULL_STATE_DICT, cfg):
                ...
                if torch.distributed.get_world_size() > 1:
                    state_dict = torch.distributed.broadcast_object_list([state_dict], src=0)[0]
        ...
        print(f"Loaded checkpoint from {checkpoint_path} (sharded={metadata['is_sharded']})")
